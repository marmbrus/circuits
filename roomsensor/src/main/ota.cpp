...
static void mark_app_valid() {
    ...
    if (esp_ota_get_state_partition(NULL, &ota_state) == ESP_OK) {
        ...
        ESP_LOGW(TAG, "Failed to get OTA state");
    }
}
...
static bool parse_manifest_and_check_update(char *manifest_data) {
    ...
    ESP_LOGI(TAG, "Manifest info - version: %s, build time: %s", remote_version, timestamp);
    ...
    // Check if we need to update (hashes don't match)
    bool update_needed = false;
    if (strlen(extracted_hash) == 0) {
        ESP_LOGI(TAG, "No current version hash set, will update");
        update_needed = true;
    } else if (strcmp(remote_hash, extracted_hash) != 0) {
        ESP_LOGI(TAG, "New version available, will update");
        update_needed = true;
    } else {
        ESP_LOGI(TAG, "Already running latest version (hash match)");

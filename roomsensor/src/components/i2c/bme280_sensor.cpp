...
bool BME280Sensor::init(i2c_master_bus_handle_t bus_handle) {
    ...
    esp_err_t err_type = add_tag_to_collection(_tag_collection, "type", "bme280");
    esp_err_t err_name = add_tag_to_collection(_tag_collection, "name", "device");
...
void BME280Sensor::poll() {
    ...
    report_metric(METRIC_PRESSURE, _pressure, _tag_collection);

    // Report humidity metric

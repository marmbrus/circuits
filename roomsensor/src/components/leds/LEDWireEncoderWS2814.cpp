...
WireEncoderWS2814::WireEncoderWS2814(int gpio, bool with_dma, uint32_t rmt_resolution_hz, size_t mem_block_symbols, size_t max_leds)
    ...
    if (err != ESP_OK) {
        ESP_LOGE(TAG_14, "led_strip_new_rmt_device failed: %s", esp_err_to_name(err));
        handle_ = nullptr;
    }
}

bool WireEncoderWS2814::transmit_frame(const uint8_t* frame_bytes, size_t frame_size_bytes) {
    ...
    if (err != ESP_OK) {
        ESP_LOGW(TAG_14, "refresh failed: %s", esp_err_to_name(err));
        return false;
    }
    busy_ = false;
    return true;
...
bool WireEncoderWS2814::is_busy() const { return busy_; }

WireEncoderWS2814::~WireEncoderWS2814() {
    if (handle_) {
        auto h = reinterpret_cast<led_strip_handle_t>(handle_);
        led_strip_del(h);
        handle_ = nullptr;

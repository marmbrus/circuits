...
static void pump_raw_to_i2s_task(void *arg) {
	...
	while (true) {
		// Idle when no pipeline/stream active
		if (!s_pipeline_running || s_raw_stream == NULL) {
		...
		if (bytes == 0 || bytes == AEL_IO_DONE || bytes == AEL_IO_ABORT) {
			...
			// Keep task alive; idle until next start
			vTaskDelay(pdMS_TO_TICKS(50));
			continue;
		}
...
static void pipeline_event_task(void *arg) {
	...
	while (true) {
		if (!s_evt) { vTaskDelay(pdMS_TO_TICKS(50)); continue; }
		if (audio_event_iface_listen(s_evt, &msg, portMAX_DELAY) != ESP_OK) { continue; }
...
static void stop_mp3_locked() {
	assert(s_pipeline_running && "stop_mp3_locked called when not running");
	assert(s_pipeline != NULL && "stop_mp3_locked requires valid pipeline");
	ESP_LOGI(TAG, "Stopping MP3 pipeline");
	// Mark as not running early to avoid race with event task
	s_pipeline_running = false;
	// Stop pipeline and detach listener before deinit
	audio_pipeline_stop(s_pipeline);
	audio_pipeline_wait_for_stop(s_pipeline);
	audio_pipeline_terminate(s_pipeline);
	if (s_evt) {
		audio_pipeline_remove_listener(s_pipeline);
		// Destroy event interface when not used to prevent stray listens
		audio_event_iface_destroy(s_evt);
		s_evt = NULL;
	}
	// Unregister and deinit elements
	if (s_http_stream) { audio_pipeline_unregister(s_pipeline, s_http_stream); audio_element_deinit(s_http_stream); s_http_stream = NULL; }
	if (s_mp3_decoder) { audio_pipeline_unregister(s_pipeline, s_mp3_decoder); audio_element_deinit(s_mp3_decoder); s_mp3_decoder = NULL; }
	if (s_raw_stream) { audio_pipeline_unregister(s_pipeline, s_raw_stream); audio_element_deinit(s_raw_stream); s_raw_stream = NULL; }
	// Deinit pipeline
	audio_pipeline_deinit(s_pipeline);
	s_pipeline = NULL;
	// Do not delete tasks; they will idle
}

static void start_mp3_locked(const char* url) {
	ESP_LOGI(TAG, "Starting MP3: %s", url);
	// Preconditions: not running, no existing pipeline or elements
	assert(!s_pipeline_running && "start_mp3_locked called while already running");
	assert(s_pipeline == NULL && s_http_stream == NULL && s_mp3_decoder == NULL && s_raw_stream == NULL && "stale pipeline/elements present before start");
	...
	if (!s_http_stream) { ESP_LOGE(TAG, "http_stream_init failed"); return; }

	mp3_decoder_cfg_t mp3_cfg = DEFAULT_MP3_DECODER_CONFIG();
	s_mp3_decoder = mp3_decoder_init(&mp3_cfg);
	if (!s_mp3_decoder) { ESP_LOGE(TAG, "mp3_decoder_init failed"); return; }

	raw_stream_cfg_t raw_cfg = RAW_STREAM_CFG_DEFAULT();
	raw_cfg.type = AUDIO_STREAM_WRITER;
	s_raw_stream = raw_stream_init(&raw_cfg);
	if (!s_raw_stream) { ESP_LOGE(TAG, "raw_stream_init failed"); return; }
	...
	while (true) {
		...
		if (want_sine) {
			...
			if (s_mode != AudioMode::Sine || hz != s_last_sine_hz) {
				if (s_pipeline_running) stop_mp3_locked();
				stop_sine_locked();
...
void audio_component_init(void) {
	...
	// Create shared event interface once (pipelines are created per start)
	audio_event_iface_cfg_t evt_cfg = AUDIO_EVENT_IFACE_DEFAULT_CFG();
	s_evt = audio_event_iface_init(&evt_cfg);
